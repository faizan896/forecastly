...
export function tornado(state, scenIdx) {
  ...
  const tweaks = [
    ...
    { key: "sgaPct", label: "SG&A % ±2pp", lo: 0.02, hi: -0.02, scen: true },
  ...
  const res = tweaks.map((t) => {
    const run = (d) => {
      ...
      if (t.scen) st.asm[t.key][scenIdx] += d; else st.asm[t.key] += d;
      return runModel(st, scenIdx).dcf.perShare;
    };
